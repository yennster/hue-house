...
struct HueAutomationService: Sendable {
    func fetchResources() async throws -> (lights: [HueLight], groups: [HueLightGroup]) {
    ...
    func setPower(on: Bool, groupID: String?) async throws -> String {
    ...
    func applyGradient(_ preset: HueGradientPreset, groupID: String?) async throws -> String {
        let (lights, groups) = try await fetchResources()
        let targetGroup = try resolveGroup(id: groupID, groups: groups)
        let targetLights = lightsForGroup(targetGroup, allLights: lights)
    ...
    func availableGroups() async throws -> [(group: HueLightGroup, lightCount: Int)] {
        let (lights, groups) = try await fetchResources()
